    ...
    if [ -d "$domain_dir" ]; then
        ...
        if [ ${#domain_files[@]} -eq 1 ]; then
            ...
            for problem_file in "${problem_files[@]}"; do
                ...
                echo "@"
                echo "Experiment name: TO-TDG"
                echo "Running experiment for: $problem_name"
                (
                    ulimit -t $TIME_LIMIT
                    ulimit -v $MEM_LIMIT
                    python3 ../__main__.py -H TDG -hp "name=\"TO-TDG\"" -tor "$domain_file" "$problem_file"
                )
                echo "@"
                ...
                (
                    ulimit -t $TIME_LIMIT
                    ulimit -v $MEM_LIMIT
                    python3 ../__main__.py -H TDG -hp "name=\"TDG\"" "$domain_file" "$problem_file"
                )
            ...
            for domain_file in "${domain_files[@]}"; do
                ...
                for problem_file in "${corresponding_problems[@]}"; do
                    ...
                    (
                        ulimit -t $TIME_LIMIT
                        ulimit -v $MEM_LIMIT
                        python3 ../__main__.py -H TDG -tor -hp "name=\"TO-TDG\"" "$domain_file" "$problem_file"
                    )
                    echo "@"
                    echo "Experiment name: TDG"
                    echo "Running experiment for: $problem_name"
                    (
                        ulimit -t $TIME_LIMIT
                        ulimit -v $MEM_LIMIT
                        python3 ../__main__.py -ml -H TDG -hp "name=\"TDG\"" "$domain_file" "$problem_file"
                    )
