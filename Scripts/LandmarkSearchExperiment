...
#folder_benchmarks="../htn-benchmarks/Monroe-Fully-Observable"
        ...
        if [ ${#domain_files[@]} -eq 1 ]; then
            ...
            for problem_file in "${problem_files[@]}"; do
                ...
                (
                    ulimit -t $TIME_LIMIT
                    ulimit -v $MEM_LIMIT
                    python3 ../__main__.py  -ml -H LMCOUNT -hp "name=\"Classical Landmarks\", use_falm=False, use_bid=False, use_disj=False" "$domain_file" "$problem_file"
                )
                echo "@"
                ...
                (
                    ulimit -t $TIME_LIMIT
                    ulimit -v $MEM_LIMIT
                    python3 ../__main__.py -ml -H LMCOUNT -hp "name=\"Bidirectional Landmarks\", use_falm=False, use_bid=True, use_disj=False" "$domain_file" "$problem_file"
                )
            ...
            for domain_file in "${domain_files[@]}"; do
                ...
                for problem_file in "${corresponding_problems[@]}"; do
                    ...
                    (
                        ulimit -t $TIME_LIMIT
                        ulimit -v $MEM_LIMIT
                        python3 ../__main__.py -ml -H LMCOUNT -hp "use_falm=False, use_bid=False, use_disj=False" "$domain_file" "$problem_file"
                    )
                    echo "@"
                    echo "Running bidirectional landmark experiment for $problem_name:"
                    (
                        ulimit -t $TIME_LIMIT
                        ulimit -v $MEM_LIMIT
                        python3 ../__main__.py -ml -H LMCOUNT -hp "use_falm=False, use_bid=True, use_disj=False" "$domain_file" "$problem_file"
                    )
