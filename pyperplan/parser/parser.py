...
from .tree_visitor import TraverseHDDLDomain, TraverseHDDLProblem, Visitable
...
class Keyword(Visitable):
    """This class represents the AST node for a hddl keyword."""
...
class Variable(Visitable):
    """This class represents the AST node for a hddl variable."""
    ...
    def __str__(self):
        return f"Var({self.name}, {self.types})"
    def __repr__(self):
        return f'VAR<{self.name}, {self.types}>'


class Type(Visitable):
    """This class represents the AST node for a hddl type."""
...
class Predicate(Visitable):
    """This class represents the AST node for a hddl predicate."""
    ...
    def __str__(self):
        # Represent parameters list in a concise way
        parameters_str = ', '.join(str(param) for param in self.parameters)
        return f"Predicate({self.name}, parameters=[{parameters_str}])"

class PredicateInstance(Visitable):
    """This class represents the AST node for a hddl predicate instance."""
...
class RequirementsStmt(Visitable):
    """This class represents the AST node for a hddl requirements statement."""
...
class DomainStmt(Visitable):
    """This class represents the AST node for a hddl domain statement."""
...
class PreconditionStmt(Visitable):
    """This class represents the AST node for a hddl action precondition."""
    ...
    def __str__(self):
        return f"PreconditionStmt({self.formula})"


class EffectStmt(Visitable):
    """This class represents the AST node for a hddl action effect."""
...
class DecomposedTaskStmt(Visitable):
    """This class represents the AST node for a hddl task binded to a method."""

    def __init__(self, name, variables):
        """Construct a new DecomposedTaskStmt.

        Keyword arguments:
        name -- name of the task decomposed by the method,
        varialbes -- list of variables
        """
        self._visitorName = "visit_decomposed_task_stmt"
        self.name= name
        self.variables= variables
    
    def __str__(self):
        var_str = ', '.join(str(v) for v in self.variables)
        return f"DecomposedTaskStmt(name={self.name}, variables={var_str})"


class Formula(Visitable):
    """
    This class represents the AST node for a hddl formula,
    ...
    def __str__(self):
        ...
        return f"Formula(key={self.key}, children=[{children_str}], type={self.type})"

class ActionStmt(Visitable):
    """This class represents the AST node for a hddl action."""
    ...
    def __init__(self, name, parameters, precond, effect):
        ...
        self.effect = effect
    def __str__(self):
        # Represent the parameters, preconditions, and effects in a concise way
        parameters_str = ', '.join(str(param) for param in self.parameters)
        #precond_str = ', '.join(str(pre) for pre in self.parameters)
        precond_str = str(self.precond)
        effect_str = str(self.effect)
        return f"Action({self.name}\n\t\t parameters=[{parameters_str}]\n\t\t precond={precond_str}\n\t\t effect={effect_str})"


class MethodStmt(Visitable):
    """This class represents the AST node for a hddl method."""

    def __init__(self, name, decompt, parameters, precond, orderedsubt):
        """Construct a new Method.

        Keyword arguments:
        name -- the name of the method
        decompt -- task which the method can decompose
        parameters -- a list of variables denoting the parameters
        precond -- the precondition of the methos given as a Formula
        subt -- subtasks of the methods
        """
        self._visitorName = "visit_method_stmt"
        self.name = name
        self.parameters = parameters
        self.precond = precond  
        self.decomposed_task = decompt
        self.ordered_subtasks = orderedsubt #Formula
    
    def __str__(self):
        result = []
        result.append(f'METHOD[')
        result.append(f'\tname={self.name}')
        result.append('\tparams=[' + ' '.join(str(p) for p in self.parameters) + ']')
        result.append('\t' + str(self.decomposed_task))
        if self.precond:
            result.append('\t' + str(self.precond))
        result.append('\t' + str(self.ordered_subtasks))
        result.append(']')
        return '\n'.join(result)
    def __repr__(self):
        return str(self)

class OrderedSubtasksStmt(Visitable):
    def __init__(self, formula, parameters):
        """Construct a new OrderedSubtasks.

        Keyword arguments:
        formula -- the list of subtasks including primite and abstract
        """
        self._visitorName = "visit_ordered_subtasks"
        self.formula = formula
        self.helper_params =  parameters #here is a helper variable for matching types with subtasks
    def __str__(self):
        return f"OrderedSubtasks({self.formula} \n\t HELPER[{self.helper_params}])"

class TaskStmt(Visitable):
    """This class represents the AST node for a hddl task."""
    ...
    def __init__(self, name, parameters=None):
        ...
        parameters -- a list of variables denoting the parameters
        precond -- the precondition of the task given as a Formula
        """
        self._visitorName = "visit_task_stmt"
        self.name = name
        self.parameters = parameters #TODO: maybe it an optional attribute
        
    def __str__(self):
        # Represent parameters list in a concise way
        parameters_str = ', '.join(str(param) for param in self.parameters)
        return f"Task({self.name}, parameters=[{parameters_str}])"

class PredicatesStmt(Visitable):
    """Represents the AST node for a hddl domain predicates definition."""
...
class DomainDef(Visitable):
    ...
    def __init__(
        ...
        if actions == None:
            ...
            self.tasks = tasks  # a list of ActionStmt
...
class ProblemDef(Visitable):
    """This class represents the AST node for a hddl domain."""

    def __init__(self, name, domainName, htn=None, objects=None, init=None, goal=None):
        ...
        htn -- an initial task netwrok with parameters
...
class Object(Visitable):
    """This class represents the AST node for a hddl object."""
...
class InitStmt(Visitable):
    """
    This class represents the AST node for a hddl problem initial condition.
    """
...
class HtnStmt(Visitable):
    """
    This class represents the AST node for a hddl problem initial condition.
    """

    def __init__(self, ordered_subtasks):
        """Construct a new HtnStmt AST node.

        Keyword arguments:
        
        """
        self._visitorName = "visit_htn_stmt"
...
class GoalStmt(Visitable):
    ...
    def __init__(self, formula):
        ...
        self.formula = formula
    def __str__(self):
        return f'GoalStmt({self.formula})'
...
def parse_formula(iter):
    ...
    """
    #NOTE: preconditions, effects can be null
    #if iter.empty:
    #    return Formula('', [], '')
    ...
    if iter.is_structure():
        ...
        # empty formula
        if p == None:
           return Formula(None, [],TypeConstant)
        key = p.get_word()
        next(iter)
...
def _parse_precondition_or_effect_or_subtasks(iter, keyword, type):
    """Parse an action precondition, effect or subtask

    Returns a PreconditionStmt, EffectStmt or OrderedSubtasksStmt instance.
    """
    if not iter.try_match(keyword):
...
def parse_precondition_stmt(it):
    return _parse_precondition_or_effect_or_subtasks(it, ":precondition", PreconditionStmt)
...
def parse_task_stmt(iter):
    """
    Parse a task definition which consists of a name, parameters, and a
    precondition.

    Returns a TaskStmt instance.
    """
    if not iter.try_match(":task"):
        raise ValueError('Error: task must start with ":task" keyword!')
    name = parse_name(iter, "task")
    param = parse_parameters(iter)
    return TaskStmt(name, param)

def _parse_binding_task(iter, params_helper):
    """
    Helper function

    Parse a task attached to a method, here there only the task name followed by variables
    params_helper is a necessary to match types with variables
    type matching is necessary here because in the visitor the variables wouldn't have access to variable types without consulting Method.parameters

    Returns a DecomposedTaskStmt instance.
    """
    key = iter.peek().get_word()
    next(iter)
    # check if task_name is in the list of tasks
    if key[0] in reserved:
        raise ValueError("Error: Binding task must not start with reserved " "char!")
    param_vars = parse_list_template(parse_variable_or_constant, iter)
    
    for i in range(len(param_vars)):
        p = param_vars[i]
        if not p.name in params_helper:
            raise ValueError(f"Error: Parameter not defined in decomposed task {key[0]}{p[0]}  " "char!")
        else:
            param_vars[i] = params_helper[p.name]
    
    return DecomposedTaskStmt(key, param_vars)
    

def parse_variable_or_constant(iter):
    """
    Helper function for pasing variables or constants linked to a task of a method

    return Varialbe or a constant 
    """       
    # non nested formula
    key = iter.get_word()
    if key[0] == "?":
...
def _parse_decomposition_task(iter, params):
    """
    Parse a task attached to a method, here there only the task name followed by variables
    params_helper is a necessary to match types with variables
    
    TODO: not sure if we couldnt merge with _parse_binding_task(next(iter), params) need to find a way for doing that
    Returns a DecomposedTaskStmt instance.
    """
    if not iter.try_match(':task'):
...
def _parse_method_precondition_ifexist(iter):
    ...
    """
    if not iter.try_match(':precondition'):
        return None
    return PreconditionStmt(parse_formula(next(iter)))


def parse_method_stmt(iter, tasks_lst, actions_lst=None):
    """
    Parse a method definition which consists of a name, the task it decomposes,
    parameters, precondition, ordered-subtasks.
    
    Returns a MethodStmt instance.
    """
    if not iter.try_match(":method"):
        raise ValueError('Error: method must start with ":method" keyword!')
    
    # method name
    name = parse_name(iter, "method")
    # parameters
    param = parse_parameters(iter)
    
    #NOTE: For now is the solution I found for type matching with variables outside parameters
    #NOTE: tmp_param_var is a helper variable for decomposed task and maybe ordered subtasks
    #NOTE:      in tree visitor we need to know the type of the variable and the variables in the subtasks were isntanciated without types
    #NOTE:      So would be not possible to verify if the arguments match the types on task declaration
    param_var_helper = {}
    for p in param:
        param_var_helper[p.name] = p
...
def parse_domain_def(iter):
    ...
    while not iter.empty():
        ...
        if key.name == "requirements":
            ...
            tasks = parse_task_stmt(next_iter)
            domain.tasks.append(tasks)
...
def parse_problem_def(iter):
    ...
    htn = parse_htn_stmt(next(iter))
    init = parse_init_stmt(next(iter))
    #NOTE: not sure how this could be done in order to make the 'goal stmt' optional
    goal = None # goal declaration is optional
    if not iter.empty() and iter.peek_tag() == ":goal":
        goal = parse_goal_stmt(next(iter))
    
    # assert end is reached
    iter.match_end()
    # create new ProblemDef instance
    return ProblemDef(probname, dom.name, htn, objects,init, goal)


def parse_htn_stmt(iter):
    """
    Parse a htn initial task netwrok, which is a string, preceded by the ':htn'
    keyword.

    Returns HTNStmt instance.
    """
    if not iter.try_match(":htn"):
        raise ValueError("Error found invalid keyword when parsing HtnStmt")
    
    #TODO: actually use parameters
    if iter.try_match(":parameters"):
        params = parse_typed_var_list(next(iter))
        
    
    ordered_subtasks = _parse_ordered_subtasks_stmt(iter, params_var_helper={})
    return HtnStmt(ordered_subtasks)
...
def parse_init_stmt(iter):
    ...
    if not iter.try_match(":init"):
        raise ValueError(f"Error found invalid keyword when parsing InitStmt {iter}")
...
class Parser:
    ...
    def parse_problem(self, dom, read_from_file=True):
        ...
        # initialize the translation visitor
        visitor = TraverseHDDLProblem(dom)
        # and traverse the AST
